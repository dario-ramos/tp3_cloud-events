...
class CheckAttendanceController(webapp2.RequestHandler):
    ...
    def renderAttendanceConfirmationScreen(self, attendance, eventName, guestEmail):
        ...
        if attendance is None:
            result = "NOT"
        self.response.out.write('<h1> Attendance ' + result + ' confirmed by guest ' +
                                guestEmail + ' for event ' + eventName + '</h1>')
    ...
    def renderEventDoesNotExistScreen(self, eventName):
        ...
        self.response.out.write(
            '<h1> Event ' + eventName + ' not registered in system! </h1>')
        self.response.out.write('<a href="/"> Back to portal </a>')
        self.response.out.write('</html></body>')
    ...
    def post(self):
        ...
        if event is None:
            ...
            return
        attendance = environment.MODEL.getAttendanceRepository(
        ).getByEventNameAndGuestEmail(eventName, guestEmail)
        self.renderAttendanceConfirmationScreen(
            attendance, eventName, guestEmail)
